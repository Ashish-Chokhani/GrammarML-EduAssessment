...
subprocess.call(['bash', 'modify_post.sh', str(var1[0]),str(sub_tag)]);
                    
# Creating segment information list, aligned phones list & transition_id's list 
var2 = [path + '/reqd_files/' + sys.argv[2]];
subprocess.call(['bash', 'extract_from_alignments.sh', str(var2[0]),str(sub_tag)]);
               
with open(path + '/reqd_files/'+sub_tag+'_tmp_segments.txt','r') as f:
    x = f.readlines();
number_of_segments = [int(tmp.split(' ')[0]) for tmp in x]; 
                      
with open(path + '/reqd_files/'+sub_tag+'_tmp_t_ids.txt','r') as f:
    x = f.readlines();
transition_id = [int(tmp.rstrip().split(' ')[0]) for tmp in x];

with open(path + '/reqd_files/'+sub_tag+'_tmp_phones.txt','r') as f:
    x = f.readlines();
aligned_phones = [tmp.rstrip().split(' ')[0] for tmp in x];

with open(path + '/reqd_files/'+sub_tag+'_posterior.txt','r') as f:
    x = f.readlines();
posterior = [tmp.rstrip().split(' ') for tmp in x];
num_of_senones = len(posterior[0]);
total_num_frames = len(posterior);            

print(num_of_senones,total_num_frames)        
...
for x in range(len(number_of_segments)):
    ...
    for y in range(len(req_t_id)-1):
        ...
        # print(tmp_post)
        score = score + math.log(tmp_prob) + tmp_post;
    
    tmp_pdf = int(lookup_tab[req_t_id[-1]-1][1]);
    tmp_post = float(posterior[cum_number_of_segments[x+1]-1][tmp_pdf]);
    score = (score + tmp_post + float(number_of_segments[x]-1)*math.log(num_of_senones)) / float(number_of_segments[x]);
